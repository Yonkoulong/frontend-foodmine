...
export class AddTaskComponent {
  ...
  onLocalClick(event: Event | any) {
    if (
      this.subtaskEdited &&
      event.target.className !== 'form-edit-sub-task' &&
      event.target.parentElement.className !== 'form-edit-sub-task'
    ) {
      const indexOfSubTaskEdited = this.subTasks.findIndex(
        (subTask) => subTask.id == this.subtaskEdited.id
      );

      const inputSubTaskEl = this._elementRef.nativeElement.querySelector(
        '.subtask-editing-ipt'
      );
      if (!inputSubTaskEl) {
        return;
  ...
  ngOnInit(): void {
    this.taskForm = this.formBuilder.group({
      ...
      endDate: [
        ...
        [Validators.required, createDatePickerValidator()],
      ],
    });
  ...
  handleChangeTaskValue() {
    if (this.taskForm.status != 'INVALID') {
      if (!this.handleCheckDueDate(this.taskForm.value.endDate)) {
        return;
      }
      ...
      if (this.isEmptyObject(this.data)) {
        ...
        //Add
        this.fireBaseTaskService.createTask({...this.taskForm.value, subTasks: this.subTasks}).subscribe({
          next: () => this.dialogRef.close(this.taskForm),
          error: (error) => console.log(`Error: ${error}`),
          complete: () => console.info('complete')
        });
  ...
  handleCheckDueDate(endDate: Date) {
    if (Date.now() > new Date(endDate).getTime()) {
      this.formState.endDate.setErrors({
        incorrect: true,
  ...
  handleUpdateStateSubtaskByCheckbox(indexSubtask: number) {
    this.subTasks[indexSubtask].completed =
      !this.subTasks[indexSubtask].completed;
  }
